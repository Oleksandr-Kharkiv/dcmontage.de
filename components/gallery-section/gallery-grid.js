'use client';
// Клієнтський компонент — потрібен для лайтбокса (useState, обробники кліків).
// yet-another-react-lightbox — легка бібліотека для перегляду фото на повний екран.
...
// Формат для лайтбокса — масив об'єктів {src}
const slides = PHOTOS.map(({ src }) => ({ src }));

export default function GalleryGrid() {
  // index — індекс відкритого фото (-1 означає закритий лайтбокс)
  ...
  return (
    <>
      ...
      {/* Лайтбокс — відкривається при кліку на фото */}
      <Lightbox
        open={index >= 0}
        index={index}
        slides={slides}
